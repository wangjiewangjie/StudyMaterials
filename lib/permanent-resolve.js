...
// 解析「线路一」及后续线路：匹配 线路[中文数字]\n域名
const LINE_RE = /线路(一|二|三|四|五|六|七|八|九|十|[0-9]+)[^\n，,。#a-z0-9]*\n?\s*([a-z0-9._-]+\.[a-z]{2,})/gi;
...
function parseLines(text) {
  ...
  while ((m = LINE_RE.exec(text)) !== null) {
    ...
    if (!host || lines.some((l) => l.host === host)) continue; // 去重同名线路
    const order = /^\d+$/.test(label) ? parseInt(label, 10) : (CN_NUM[label] || 99);
    lines.push({ label: `线路${label}`, order, host });
  }
